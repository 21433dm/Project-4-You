...
if (cl_Login::clIsLoggedIn()) {
            ...
            if (password_verify($oldpass, DB::query('SELECT password FROM cl_users WHERE id=:id', array(':id'=>$userid))[0]['password'])) {
                        if ($newpass == $newconfirm) {
                                if (strlen($newpass) >= 6 && strlen($newpass) <= 60) {
                                        DB::query('UPDATE cl_users SET password=:newpass WHERE id=:id', array(':newpass'=>password_hash($newpass, PASSWORD_BCRYPT), ':id'=>$userid));
